...
try {
    ...
    die(error_page("Práctica 8","<p>No se ha podido conectar la BD: ".$e->getMessage()."</p>"));
}
...
if(isset($_POST["btnContBorrar"])){
    try{
        ...
        session_destroy();
        die(error_page("Práctica 8","<p>No se ha podido reallizar la consulta: ".$e->getMessage()."</p>"));
    }
}
if(isset($_POST["btnDetalles"]) || isset($_POST["btnBorrar"])){
    ...
    try{
        $consulta =" select * from usuario s where id_usuario='".$id_usuario."'";
...
if(isset($_POST["btnContAgregar"])){
    ...
    if(!$error_dni){
        ...
        if(is_string($error_dni)){
            mysqli_close($conexion);
            session_destroy();
            die(error_page("Practica 8","<p>".$error_dni."</p>"));
        }
    }
}
...
if(!$error_form_agregar){
    ...
    try{
        ...
        mysqli_close($conexion);
        session_destroy();
        die(error_page("Practica 8","<p>No se ha podido realizar la consulta: ".$e->getMessage()."</p>"));
    }

    $_SESSION["mensaje_accion"]="Usuario insertado con exito";
    if($_FILES["foto"]["name"]!=""){
        ...
        if($var){
            try{
                $consulta="update usuarios set foto='".$nombre_nuevo."' where id_usuario='".$ultim_id."'";
                mysqli_query($conexion,$consulta);
            }
            catch(Exception $e){
