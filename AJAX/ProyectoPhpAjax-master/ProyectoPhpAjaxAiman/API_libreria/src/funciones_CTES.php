...
function actualizar_libro($datos_actualizar)
{
    try {
        $conexion = new PDO("mysql:host=" . SERVIDOR_BD . ";dbname=" . NOMBRE_BD, USUARIO_BD, null, array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES 'utf8'"));
    } catch (PDOException $e) {
        $respuesta["error"] = "No he podido conectarse a la base de batos: " . $e->getMessage();
        return $respuesta;
    }
    ...
    try {
        $consulta = "update libros set titulo=?, autor=?, descripcion=?, precio=? , portada=? where referencia=?";
        $sentencia = $conexion->prepare($consulta);
        $sentencia->execute($datos_actualizar);
    } catch (PDOException $e) {
        $sentencia = null;
        $conexion = null;
        $respuesta["error"] = "No he podido realizarse la consulta: " . $e->getMessage();
    ...
    $respuesta["mensaje"] = "El libro (" . end($datos_actualizar) . ") ha sido actualizado correctamente";
    $sentencia = null;
    $conexion = null;
    return $respuesta;
}

function insertar_libro($datos_insert)
{
    try {
        $conexion = new PDO("mysql:host=" . SERVIDOR_BD . ";dbname=" . NOMBRE_BD, USUARIO_BD, null, array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES 'utf8'"));
    } catch (PDOException $e) {
