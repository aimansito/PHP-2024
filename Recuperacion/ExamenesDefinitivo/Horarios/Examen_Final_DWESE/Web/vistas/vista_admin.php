<?php
    if(isset($_POST["btnDetalles"])){
        $url=DIR_SERV."/obtenerUsuario/". $_POST["btnDetalles"];
        $respuesta=consumir_servicios_JWT_REST($url,"GET",$headers);
        $jon_detalles=json_decode($respuesta,true);
        if(!$jon_detalles){
            session_destroy();
            die(error_page("Examen Final PHP","<h1>Examen Final PHP</h1><p>Error consumiendo servicio: ".$url."</p>"));
        }
        if(isset($jon_detalles["error"])){
            session_destroy();
            die(error_page("Examen Horarios PHP","<h1>Examen Horarios PHP</h1><p>".$jon_detalles["error"]."</p>"));
        }
    ...
    for($hora=1;$hora<=count(HORAS);$hora++){
        if($hora!=4){
            $url=DIR_SERV."/usuariosGuardia/"."5"."/".$hora;
            $respuesta=consumir_servicios_JWT_REST($url,"GET",$headers);
            ...
            if(isset($jon_profesores_guardia["error"])){
                session_destroy();
                die(error_page("Examen Horarios PHP","<h1>Examen Horarios PHP</h1><p>".$jon_profesores_guardia["error"]."</p>"));
            }
...
<head>
    ...
    <style>
        ...
        th{background-color: #CCC;padding: 0.5rem;}
    </style>
</head>
    ...
    <?php   
        ...
        for($hora=1;$hora<=count(HORAS);$hora++){
            if($hora!=4){
                ...
                echo "<td>";
                echo "<ol>"; 
                foreach($jon_profesores_guardia["usuarios"] as $tupla){
