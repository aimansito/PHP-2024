<?php
    ...
    if(!$json_horario){
        session_destroy();
        die(error_page("Examen Horarios","<h1>Examen Final</h1><p>Error consumiendo servicio:".$url."</p>"));
    }
    if(isset($json_horario["error"])){
        session_destroy();
        die(error_page("Examen Horarios","<h1>Exame Final PHP</h1><p>".$json_horario["error"]."</p>"));
    }
