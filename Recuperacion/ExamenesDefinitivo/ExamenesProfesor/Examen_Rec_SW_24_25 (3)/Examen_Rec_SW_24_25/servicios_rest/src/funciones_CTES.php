...
function datos_usuarios($id_usuario)
{
    try{
        $conexion=new PDO("mysql:host=".SERVIDOR_BD.";dbname=".NOMBRE_BD,USUARIO_BD,CLAVE_BD,array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES 'utf8'"));
    }
...
function usuarios_guardia($dia,$hora)
{
    ...
    try{
        $consulta="select usuarios.* from horario_guardias,usuarios where usuarios.id_usuario=horario_guardias.usuario and dia=? and hora=?";
        $sentencia=$conexion->prepare($consulta);
...
function de_guardia($id_usuario)
{
    try{
        $conexion=new PDO("mysql:host=".SERVIDOR_BD.";dbname=".NOMBRE_BD,USUARIO_BD,CLAVE_BD,array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES 'utf8'"));
    }
