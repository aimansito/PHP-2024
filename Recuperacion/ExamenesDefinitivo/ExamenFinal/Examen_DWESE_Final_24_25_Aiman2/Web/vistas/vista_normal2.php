<?php
    ...
    if(!$json_horario_profesor){
        session_destroy();
        die(error_page("Examen Final PHP","<h1>Examen Final</h1><p>Error consumiendo servicio rest: '.$url.'</p>"));
    }
    if(isset($json_horario_profesor["error"])){
         session_destroy();
         die(error_page("Examen Final PHP","<h1>Examen Final</h1><p>".$json_horario_profesor['error']."</p>"));
    }
    ...
    if(isset($json_horario_profesor["mensaje_baneo"])){
        session_unset();
        $_SESSION["mensaje_seguridad"]="Usted ya no se encuentra disponible en la BD";
        header("Location:index.php");
        exit;
    }
...
<body>
    ...
    <div>
        Bienvenido <strong><?php echo $datos_usu_log["usuario"] ; ?></strong> <form class="enlinea" action="index.php" method="post"><button type="submit" class="enlace" name="btnSalir">Salir</button></form>
    </div>
    <?php
        echo "<h3>Horario profesor: ".$datos_usu_log["nombre"]."</h3>";
