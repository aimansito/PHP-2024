...
function obtenerLibro($referencia)
{
    try {
        $conexion = new PDO("mysql:host=" . SERVIDOR_BD . ";dbname=" . NOMBRE_BD, USUARIO_BD, CLAVE_BD, array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES 'utf8'"));
    } catch (PDOException $e) {
...
function crearLibro($datos_añadir)
{
    try {
        $conexion = new PDO("mysql:host=" . SERVIDOR_BD . ";dbname=" . NOMBRE_BD, USUARIO_BD, CLAVE_BD, array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES 'utf8'"));
    } catch (PDOException $e) {
    ...
    try {
        ...
        $sentencia = $conexion->prepare($consulta);
        $sentencia->execute($datos_añadir);
    } catch (PDOException $e) {
        $sentencia = null;
        $conexion = null;
        $respuesta["error"] = "No he podido realizarse la consulta: " . $e->getMessage();
    ...
    $respuesta["mensaje"] = "Libro añadido correctamente en la BD";
    $sentencia = null;
    $conexion = null;
    return $respuesta;
}
...
function repetido($tabla, $columna, $valor)
{
    ...
    $respuesta["repetido"] = $sentencia->rowCount() > 0;// si me devuelve un dato, sera true en otro caso sera false.
    $sentencia = null;
    $conexion = null;
    return $respuesta;
}
...
function actualizarLibro($datos_editar)
{
    try {
        $conexion = new PDO("mysql:host=" . SERVIDOR_BD . ";dbname=" . NOMBRE_BD, USUARIO_BD, CLAVE_BD, array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES 'utf8'"));
    } catch (PDOException $e) {
