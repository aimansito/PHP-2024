<?php
    ...
    if(isset($_POST["dia"])){
        $headers[]="Authorization: Bearer " . $_SESSION["token"];
        $url = DIR_SERV . "/usuariosGuardias/" . $_POST["dia"] . "/" . $_POST["hora"];
        $respuesta = consumir_servicios_JWT_REST($url,"GET",$headers);
        ...
        if(isset($json_usuarios["error"])){
            session_destroy();
            die(error_page("Examen Final","<h1>Gestión de Guardias</h1><p>".$json_usuarios["usuarios"]."</p>"));
        }
...
<body>
    ...
    <div>
        Bienvenido <strong><?php echo $datos_usu_log["usuario"]; ?></strong><form class="enlinea" method="post" action="index.php"><button class="enlace" type="submit" name="btnSalir">Salir</button></form>
    </div>
    ...
    <?php
        ...
        echo "<h3>" . $dias[$_POST["dia"]]. " a ". $_POST["hora"] . "º</h3>";
