<?php
    if(isset($_POST["btnContEditar"])){
        ...
        if(!$_FILES["portada"]["name"]==""){
            $error_portada=!getimagesize($_FILES['portaada']['tmp_name']) || $_FILES["portada"]["error"] || $_FILES["portada"]["size"] < 500 * 1024;
        ...
        if(!$errores_form){
            ...
            if(!$respuesta_actualizar){
                session_destroy();
                die(error_page("Examen Libreria","Error no se ha podido hacer el servicio rest"));
            }
            if(isset($respuesta_actualizar['error'])){
                session_destroy();
                die("Error realizar el servicio" . $respueta_actualizar['error']);
            }
            ...
            if(!$_FILES["portada"]["name"]==""){
                ...
                if(!$file){
                    ...
                    if(!$respuesta_portada){
                        session_destroy();
                        die(error_page("Examen Libreria SW 24-25","Error. no se ha podido hacer el servicio rest"));
                    }
    ...
    if(isset($_POST["btnContAgregar"])){
        ...
        if(!$error_referencia){
            ...
            if(!$respuesta_repetido){
                session_destroy();
                die(error_page("Examen Libreria SW 24-25","Error. no se ha podido hacer el servicio rest de repetido"));
            }
        ...
        if(!$_FILES["portada"]["name"]==""){
            $error_portada = !getimagesize($_FILES['portada']['error']) || $_FILES["portada"]["error"] || $_FILES["portada"]["size"] > 500 * 1024;
            $errores_form = $error_referencia || $error_titulo || $error_auto || $error_descripcion || $error_precio || $error_portada;
        }else{
    ...
    if(isset($_POST["btnEditar"])){
        ...
        if(isset($json_detalles['error'])){
            session_destroy();
            die("Error realizando servicio".$json_detalles['error']);
        }
    }
...
<body>
    ...
    <?php
        if(isset($_SESSION['mensaje_acc'])){
            ...
            // Igualo $_POST a predefinido para que no se guarden los datos en agregar
            $_POST=[];
