<?php
    if(isset($_POST["btnLogin"])){
        ...
        if(!$error_form_login){
            ...
            if(!$json_respuesta){
                session_destroy();
                die(error_page("Examen Guardias","<h1>Guardias</h1><p>Error al consumir el servicio: <strong>".$url."</strong></p>"));
            }
            if($json_respuesta["error"]){
                session_destroy();
                die(error_page("Examen Guardias","<h1>Guardias</h1><p>".$json_respuesta["error"]."</p>"));
            }
