...
function login($datos_login){
    ...
    if($sentencia->rowCount()>0){
        ...
        $payload['exp'] = time() + (MINUTOS_API * 60);
        $payload['data']=$respuesta["usuario"]["id_usuario"];
        $jwt = JWT::encode($payload,PASSWORD_API,"HS56");
        $respuesta["token"] = $jwt;
    }else{
...
function horariosProfesor($id_usuario){
    try{
    ...
    try{
        $consulta = "SELECT horario_lectivo.dia, horario_lectivo.hora. grupos.nombre as grupo, aulas.nombre as aula 
...
function horarioGrupo($id_grupo){
    ...
    try{
        $consulta = "SELECT horario_lectivo.dia,horario_lectivo.hora, usuarios.usuario. aulas.nombre as aula
                    FROM horario_lectivo
                    JOIN grupos ON horario_lectivo.grupo = grupos.id_grupo
                    JOIN aulas ON horario_lectivo.aula = aulas.id_aula
                    JOIN usuarios ON horario_lectivo.usuarios = usuarios.id_usuario
    ...
    $sentencia["horario"]=$sentencia->fetchAll(PDO::FETCH_ASSOC);
    $sentencia=null;
    $conexion=null;
    return $respuesta;
}
...
function profesoresLibres($dia,$hora,$id_grupo){
    ...
    try{
        $consulta = "SELECT usuarios.id_usuario, usuarios.nombre
                    ...
                    (SELECT horario_lectivo.usuario
                    FROM horario_lectivo
...
function profesoresOcupados($dia, $hora, $id_grupo){
    ...
    try{
        $consulta = "SELECT usuarios.id_usuario, usuarios.usuario, aulas.,nombre
                    FROM usuarios
                    JOIN horario_lectivo ON horario_lectivo.usuario = usuarios.id_usuario
                    JOIN aulas ON horario_lectivo.aula = aulas.id_aula
                    WHERE usuarios.tipo <> 'admin'
                    AND horario_lectivo.dia = ?
                    AND horario_lectivo.hora=?
                    AND horario_lectivo.grupo=?";
        $sentencia=$conexion->prepare($consulta);
        $sentencia->execute([$dia,$hora,$id_grupo]);
    }catch(PDOException $e){
        $sentencia = null;
    ...
    $respuesta["profesores_ocupados"]=$sentencia->fetchAll(PDO::FETCH_ASSOC);
    $sentencia=null;
    $conexion=null;
    return $respuesta;
}
