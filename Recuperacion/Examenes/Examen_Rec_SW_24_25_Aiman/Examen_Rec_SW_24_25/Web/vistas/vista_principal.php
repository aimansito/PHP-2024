<?php
    if(isset($_POST["id_usuario"])){
        ...
        if(isset($json_usuario["no-auth"])){
            session_unset();
            $_SESSION["mensaje_seguridad"]="El tiempo de sesion de la API ha expirado";
            header("Location:index.php");
            exit;
        }
        if(isset($json_usuario["mensaje-baneo"])){
            session_unset();
    ...
    if(isset($_POST["dia"])){
        ...
        $respuesta = consumir_servicios_JWT_REST($respuesta,"GET",true);
        ...
        if(isset($json_usuarios["no-auth"])){
            session_unset();
            $_SESSION["mensaje_seguridad"] = "El tiempo de sesion de la API ha expirado";
            header("Location:index.php");
            exit;
    ...
    if(isset($json_respuesta["mensaje_baneo"])){
        session_unset();
        $_SESSION["mensaje_seguridad"]="Usted ya no se encuentra la bd";
        header("Location:index.php");
        exit;
    }
...
<body>
    ...
    <div>
        ...
        <form class="enlinea" action="index.php" method="post"><button class="enlace" type="submit" name="btnSalir">Salir</button></form>
    </div>
    ...
    <?php
        ...
        if(isset($_POST["dia"])){
            ...
            $primera_fila = true;
            foreach($json_usuarios["usuarios"] as $tuplas){
                echo "<tr>";
                echo "<td><form action='index.php' method='post'>";
                ...
                if($primera_fila){
                    ...
                    if(isset($_POST["id_usuario"])){
                        ...
                        echo "<p class='text_izq'><strong>Usuario: </strong>".$json_usuario["usuario"]["nombre"]."</p>";
                        echo "<p class='text_izq'><strong>Contraseña: </strong></p>";
                        echo "<p class='text_izq'><strong>Email: </strong></p>";
